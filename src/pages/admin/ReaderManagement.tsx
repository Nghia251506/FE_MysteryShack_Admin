...
export default function ReaderManagement() {
  ...
  const filteredReaders = useMemo(() => {
    // Nếu đang loading hoặc chưa có data thì trả về mảng rỗng
    if (!users || !Array.isArray(users)) return [];

    return users.filter(r => {
      // 1. Kiểm tra tên (An toàn hơn với optional chaining)
      ...
      // 3. Elo Range (Chỉ filter nếu có nhập số)
      const minElo = filterEloMin ? parseInt(filterEloMin) : -99999;
      const maxElo = filterEloMax ? parseInt(filterEloMax) : 99999;
      const matchElo = (r.eloScore || 0) >= minElo && (r.eloScore || 0) <= maxElo;

      // 4. Date Range (Tạm thời bỏ qua nếu date không hợp lệ để hiện data đã)
      let matchDate = true;
      if ((filterDateFrom || filterDateTo) && r.createdAt) {
        ...
        matchDate = !isNaN(joinDate.getTime()) && joinDate >= from && joinDate <= to;
      }
  ...
  const confirmEloAdjustment = () => {
    ...
    setReaders(Headers.map(r => r.id === adjustEloReader.id ? { ...r, elo: newElo } : r));
    const newLog: EloLog = {
      ...
      <div className="bg-white p-1.5 rounded-2xl shadow-sm border border-gray-100 inline-flex mb-8 overflow-x-auto max-w-full">
        {[
          ...
          <button
            ...
            className={`px-5 py-2.5 rounded-xl text-sm font-bold flex items-center gap-2 transition-all duration-300 whitespace-nowrap ${activeTab === tab.id
